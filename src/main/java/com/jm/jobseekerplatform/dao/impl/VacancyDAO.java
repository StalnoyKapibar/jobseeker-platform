...
public class VacancyDAO extends AbstractDAO<Vacancy> {
    ...
    private Page<Vacancy> getVacancies(Query query, Set<Tag> tags, int limit, int page) {
        long totalElements = (long) entityManager
                .createQuery("select count(distinct v) from Vacancy v join v.tags t where v.state='ACCESS'" +
                        " and t in (:tags)")
        ...
        List<Vacancy> vacancyList = entityManager.createQuery("select v from Vacancy v where v.id in (:ids)")
    ...
    public Page<Vacancy> getVacanciesSortedByCityTagsViews(long seekerId, String city, int limit, int page) {
        ...
        Query query = (Query) entityManager.createNativeQuery(
                "Call getSortedVacs(:seekerId, :city, :limitFrom, :limitTo)", Vacancy.class);
    ...
    public Vacancy getById(Long id) {
        return entityManager.createQuery("select v from Vacancy v where v.id=:id", Vacancy.class)
                .setParameter("id", id)
    ...
    public List<Vacancy> getTrackedByEmployerProfileId(Long id) {
        ...
        Query query = entityManager.unwrap(Session.class)
                .createSQLQuery("SELECT * FROM (SELECT * FROM vacancies where publication_position " +
                        "IS NOT NULL ORDER BY publication_position)as t where employer_profile_id = :id")
    ...
    public List<Vacancy> getAllVacanciesByTagName(String tagName) {
        Query query = entityManager.createQuery("SELECT distinct v FROM Vacancy v JOIN v.tags vt " +
