...
function searchSeeker(string) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each(data, function (i, seekerUser) {
                let bool = seekerUser.enabled ? 'display:none' : '';
                let boolfalse = seekerUser.enabled ? '' : 'display:none';
                let insert = '' +
                    '<div id="userBlockId' + seekerUser.id + '">' +
                        '<button class="btn btn-success user_unblock adminEnableUserBtn" style="' + bool + '"' +
                                'data-id="' + seekerUser.id + '">Разблокировать<br>' +
                            '<span id="timeEndTheLock_' + seekerUser.id + '"></span>' +
                        '</button>' +
                        '<div class="dropdown user_block" style="margin:0; ' + boolfalse + '">' +
                            ...
                            '<div class="dropdown-menu">' +
                                '<a class="dropdown-item user_block_operation" data-id="' + seekerUser.id + '"' +
                                        'data-period="1" data-email="' + seekerUser.email + '">На сутки</a>' +
                                '<a class="dropdown-item user_block_operation" data-id="' + seekerUser.id + '"' +
                                        'data-period="3" data-email="' + seekerUser.email + '">На 3 дня</a>' +
                                '<a class="dropdown-item user_block_operation" data-id="' + seekerUser.id + '"' +
                                        'data-period="7" data-email="' + seekerUser.email + '">На 7 дней</a>' +
                                '<a class="dropdown-item user_block_operation" data-id="' + seekerUser.id + '"' +
                                        'data-period="14" data-email="' + seekerUser.email + '">На 14 дней</a>' +
                                '<a class="dropdown-item user_block_operation" data-id="' + seekerUser.id + '"' +
                                        'data-period="0" data-email="' + seekerUser.email + '">Бессрочно</a>' +
                            '</div>' +
                        '</div>' +
                    '</div>' +
                    ...
                    '<tr style="height: ' + heightNum + 'px">' +
                        ...
                        '<td>' +
                            '<button onclick="editEmployer(' + seekerUser.id + ')" type="button"' +
                                ...
                                'Edit Seeker' +
                            '</button>' +
                            '<button onclick="deleteEmployer(' + seekerUser.id + ')" type="button"' +
