    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $.each(
                data.tags,
                function(key, value) {
                    tags += '<span class="badge badge-pill badge-success btnClick text-dark">' +
                        value.name + '</span>'
                });
            $("#VMTags").html(tags);
            ...
            let chat = '<button onclick="openChatByVacancy(' +
                data.id +
                ')">Связаться с работодателем</button>' +
                    ...
                    success: function(data) {
                        ...
                        if (data.length == 0) {
                            $('#search_advice_wrapper').append(
                                '<div class="advice_variant"> По запросу "' +
                                param +
                            ...
                            $.each(data, function(key, value) {
                                ...
                                if (isAdded) {
                                    $('#search_advice_wrapper')
                                        .append(
                                            '<div style="display: none" class="advice_variant"  onclick="addTagToSearch(\'' +
                                            value.id +
                                            '\',\'' +
                                            value.name +
                                            '\')">' +
                                    ...
                                    $('#search_advice_wrapper')
                                        .append(
                                            '<div class="advice_variant" onclick="addTagToSearch(\'' +
                                            value.id +
                                            '\',\'' +
                                            value.name +
                                            '\')">' +
...
function addTagToSearch(id, name) {
    $('#searchButtons')
        .append(
            '<span class="listTags" id="tagItem-' +
            id +
            '"><span style="margin:0 5px 5px 0;" class="badge badge-pill badge-success tagButton" id="searchButton-' +
            id + '" onclick="deleteButton(\'' + id + '\')">' +
            name + '</span>' +
            '<input class="tagIdH" id="tagId-' + id +
            '" type="hidden" value="' + id + '"></span>');
    $('#search_box').val('');
            ...
            success: function(data) {
                if (data.content.length === 0) {
                    if (searchBox.length === 0 && param.length > 0) {
                        $('#searchHeader').text(
                            'По запросу "' + param[0].name +
                            '" ничего не найдено');
                    } else {
                        $('#searchHeader').text(
                            'По запросу "' + searchBox +
                            '" ничего не найдено');
                    }
                } else {
                    if (param.length === 1) {
...
$(window).scroll(
    function() {
        if (Math.abs(($(document).height() - $(window).height()) -
                $(window).scrollTop()) < 50) {
            if (block = true) {
                if (page < total_pages) {
...
function blockVacancy(vacancyId, caller) {
    $.get("/api/users/get-user-authority", function(usrRole) {
        if (usrRole.authority === 'ROLE_ADMIN') {
            $.ajax({
                type: 'post',
...
function printVacancies(data) {
    ...
    adminButt = '';
    $.get(
        "/api/users/get-user-authority",
        function(usrRole) {
            if (usrRole.authority === 'ROLE_ADMIN') {
                eval('userRole = usrRole.authority');
            }
                ...
                function(key, value) {
                    ...
                    if (seekerAuthority) {
                        $.each(
                            ...
                            function(i,
                                ...
                                if (item === value.id
                                    .toString()) {
                                    favVac = '<span class="stars" id="stars-' +
                                        value.id +
                                        '"><i id="outFavorite' +
                                        value.id +
                                        '" class="fas fa-star" onclick="outFavorite(' +
                                        value.id +
                                        ',' +
                                        seekerProfileId +
                                        ');event.stopPropagation();" title="убрать из избранных"></i></span>';
                                }
                            });
                        if (favVac === '') {
                            favVac = '<span class="stars" id="stars-' +
                                ...
                                '"><i id="inFavorite' +
                                value.id +
                                '" class="far fa-star" onclick="inFavorite(' +
                                value.id +
                                ',' +
                                seekerProfileId +
                                ');event.stopPropagation();" title="в избранное"></i></span>';
                        }
                    }
                    if (value.salaryMin) {
                        minSalary = '<div class="salary"><span>Зарплата от: ' +
                            value.salaryMin +
                            ' руб.</span></div>';
                    }
                    $.each(
                        ...
                        function(i, item) {
                            if (seekerAuthority) {
                                ...
                                if (bool == true) {
                                    vacancyTags += '<span class="badge badge-pill badge-success btnClick text-dark" style="white-space: pre"><h7>' +
                                        item.name +
                                        '   </h7><i class="fas fa-tag"></i></span>';
                                } else {
                                    vacancyTags += '<span class="badge badge-pill badge-success btnClick text-dark" style="white-space: pre"><h7>' +
                                        item.name +
                                        '   </h7></span>';
                    ...
                    $('#searchList')
                        .append(
                            ...
                            ' data-target="#vacancyModal" onclick="showVacancy(\'' +
                            value.id +
                            '\')">' +
                            '<div class="headLine"><span>' +
                            value.headline +
                            '</span>' +
                            '   ' +
                            favVac +
                            ...
                            vacancyTags +
                            '</div>' +
                            '<div class="companyData"><span>Компания: ' +
                            value.creatorProfile +
                            '</span><br><span>Город: ' +
                            value.city +
                            '</span></div>' +
                            '<div class="vacancyDescription"><span>' +
                            ...
                            value.id +
                            '\';event.stopPropagation();">На страницу вакансии</span>' +
                            adminButt +
                            '</div>' +
                            '</li>');
            ...
            function check_seeker_tags(tag) {
                ...
                $.each(seeker_tags, function(i, item) {
                    var s_tag = item.name.toString().split(' ')
                        .join('').replace("/", "")
                        .toLocaleLowerCase();
                    var v_tag = tag.name.toString().split(' ')
                        .join('').replace("/", "")
                        .toLocaleLowerCase();
...
function inFavorite(vacancyId, seekerProfileId) {
    $.ajax({
        type: 'post',
        url: "/api/seekerprofiles/inFavoriteVacancy?vacancyId=" +
            vacancyId + "&seekerProfileId=" + seekerProfileId,
        contentType: 'application/json; charset=utf-8',
        beforeSend: function(request) {
            request.setRequestHeader(header, token);
        },
        success: function() {
            ...
            $('#stars-' + vacancyId)
                .append(
                    ...
                    ',' +
                    seekerProfileId +
                    ');event.stopPropagation();" title="убрать из избранных"></i>');
...
function outFavorite(vacancyId, seekerProfileId) {
    $.ajax({
        type: 'post',
        url: "/api/seekerprofiles/outFavoriteVacancy?vacancyId=" +
            vacancyId + "&seekerProfileId=" + seekerProfileId,
        contentType: 'application/json; charset=utf-8',
        beforeSend: function(request) {
            request.setRequestHeader(header, token);
        ...
        success: function() {
            ...
            $('#stars-' + vacancyId)
                .append(
                    ...
                    ',' +
                    seekerProfileId +
                    ');event.stopPropagation();" title="в избранное"></i>');
        },
        error: function(error) {
            console.log(error);
            alert(error.toString());
    ...
    $
        .ajax({
            ...
            success: function(vacancies) {
                $("#viewedVac")
                    .append(
                        '<div class="empCard" id="empCardVac" style="margin-left: 0px;">' +
                        '<div class="card mb-3">' +
                    ...
                    function(key, value) {
                        $("#viewed_vacancies")
                            .append(
                                ...
                                value.headline +
                                '</span></div>' +
                                '<div class="card-body"><h6 style="color: #777777;">Компания:</h6><span>' +
                                value.companyname +
                                '</span><br/><br/><h6 style="color: #777777;">Зарплата:</h6>' +
                                salaryFormat(
                                    value.salarymin,
                                    value.salarymax) +
                                '</div>' +
                                ...
                                ' onclick="window.location.href =\'/vacancy/' +
                                value.id +
                                '\';event.stopPropagation();">На страницу вакансии</span></div></li>');
                    })
            }
        })
}
...
function salaryFormat(salarymin, salarymax) {
    ...
    if (salarymin != 0 && salarymax != 0) {
        return salary = '<span style="white-space: pre">от:  ' + salarymin +
            ' &#8381   до:  ' + salarymax + ' &#8381</span>';
    } else if (salarymin == 0 && salarymax != 0) {
        return salary = '<span style="white-space: pre">до:  ' + salarymax +
            ' &#8381</span>';
    } else if (salarymin != 0 && salarymax == 0) {
        return salary = '<span style="white-space: pre">от:  ' + salarymin +
            ' &#8381</span>';
